...
export default async function handler(req, res) {
  ...
  const { data: job, error: jobError } = await admin
    .from("jobs")
    .select("id, name, address, contractor, reg_no, coc_ref, floor_plan_url, created_at")
    .eq("id", id)
    .single();
  ...
  const { data: cables } = await admin
    .from("cables")
    .select("cable_id, from_point, to_point, cable_type, notes, photo, photo_url, audio_url, pin_x, pin_y, created_at")
    .eq("job_id", id)
    .order("created_at");
